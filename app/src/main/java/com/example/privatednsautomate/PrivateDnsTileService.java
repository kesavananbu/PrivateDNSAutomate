...
public class PrivateDnsTileService extends TileService {
    ...
    public static String DNS_PRIMARY_ON = "primary";
    public static String DNS_SECONDARY_ON = "secondary";
    ...
    public void onStartListening() {
        ...
        if (dnsmode.equalsIgnoreCase(DNS_MODE_OFF)) {
            ...
            if ((dnsprovider != null)) {
                ...
                if(StorageHelper.GetPrimaryDNS(this) == dnsprovider)
                {
                    refreshTile(tile, Tile.STATE_ACTIVE, DNS_PRIMARY_ON, R.drawable.ic_dnson);
                }
                else if (StorageHelper.GetSecondaryDNS(this) == dnsprovider)
                {
                    refreshTile(tile, Tile.STATE_ACTIVE, DNS_SECONDARY_ON, R.drawable.ic_dnson);
                }
                else
                {
    ...
    public void ToggleDNSOn(Tile tile, String dnsprovider, Boolean togglesecondary)
    {

        if (dnsprovider != null && dnsprovider != "" && !dnsprovider.isEmpty())
        {
            if(togglesecondary && dnsprovider.equalsIgnoreCase(StorageHelper.GetPrimaryDNS(this)))
            {
                dnsprovider = StorageHelper.GetSecondaryDNS(this);
            }
            else
            {
                dnsprovider = StorageHelper.GetPrimaryDNS(this);
            }
    ...
    public void onClick() {
        ...
        final Boolean togglesecondary = togglestates.getBoolean("toggle_secondary", true);
        ...
        if (hasPermission()) {
            ...
            if (dnsmode.equalsIgnoreCase(DNS_MODE_OFF)) {
                if (toggleauto) {
                    ...
                    ToggleDNSOn(tile,dnsprovider,togglesecondary);
                    //changeTileState(tile, Tile.STATE_ACTIVE, dnsprovider, R.drawable.ic_dnson, DNS_MODE_ON);
                }
            } else if (dnsmode.equalsIgnoreCase(DNS_MODE_AUTO)) {
                if (dnsprovider != null) {
    ...
    public void changeTileState(Tile tile, int state, String label, int icon, String dnsmode) {
        ...
        Settings.Global.putString(getContentResolver(), "private_dns_specifier", label);
        tile.updateTile();
    }
